...
def gen():
    ...
    while(cap.isOpened()):
        ...
        if success == True:
            ...
            # output['detect']=results.pandas().xyxy[0].name
            # if(len(output['detect']))>0:
            #     fnl=results.pandas().xyxy[0].name.to_json()
            #     fnl= json.loads(fnl)
                # print(len(output['detect']))
                # for i in range(len(output['detect'])):
                #     print(fnl[str(i)])
            # print (results.pandas().xyxy[0].name.to_json())
            # results.pandas().xyxy[0].name.to_csv('file1.csv',header=False, index=False)

            # with open("sample.json", "w") as outfile:
            #     json.dump(fnl, outfile)
...
def gen2():
    ...
    if(len(output['detect']))>0:
        ...
        for i in range(len(output['detect'])):
            print(fnl[str(i)])
            label.append(fnl[str(i)])
        fnlrslt=listToString(label)
        rslt=dict({'label':fnlrslt})
        print(rslt)
        return jsonify(rslt)
    else:
        not_found()
